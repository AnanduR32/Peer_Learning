...
class Peer {
  addStacks(int id) {
    //Add a particular stack of interest/expertise
  ...
  void setMentorOrLearner(var id) {
    //Set whether the participant is learner or mentor
    print('Is user mentor or learner: ');
    var desig = stdin.readLineSync();
    if (desig.toLowerCase() == 'learner') {
      user[id]['desig'] = 'learner';
    } else if (desig.toLowerCase() == 'mentor') user[id]['desig'] = 'mentor';
  }

  void setAvailableTime(var id) {
    //if person is mentor set available time
  ...
  void getMentor(var id) {
    //Takes stack and time as params and finds available mentors.
    if (user[id]['desig'] == 'mentor') {
      print('Enter tech stack: ');
      var stack = stdin.readLineSync();
      stack.split(' ');
